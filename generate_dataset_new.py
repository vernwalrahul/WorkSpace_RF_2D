...
def get_path_nodes(shallow_G1, dense_G1, start_n, goal_n, obstacles):
    ...
    # to_append = []
    # for node in c_path_nodes:
    #     if(good_node(node, dense_G1, obstacles)):
    #         nearby_nodes = get_nearby_good_nodes(node, dense_G1, obstacles)
...
def main():
        ...
        while(p<no_pp):
            ...
            while(flag<5):
                ...
                if(not is_trivial(start, goal, obstacles)):
                    if(path_exists(start_n, goal_n, dense_G2)):
                        # print("inside if")
                        shallow_G1.add_node(start_n, state = dense_G2.node[start_n]['state'])
                        shallow_G1.add_node(goal_n, state = dense_G2.node[goal_n]['state'])

                        shallow_G1 = connect_knn(shallow_G1, dense_G2, start_n, goal_n, knn)
                        if(not path_exists(shallow_G1, 'o'+start_n, 'o'+goal_n)):
                            ...
                            # return
                            flag += 1
                            path_nodes = get_path_nodes(shallow_G1, dense_G2, start_n, goal_n, obstacles)
                    ...
                    # print("trivial nodes")
            if(not flag==0):        
                all_path_nodes.append(list(chain.from_iterable(curr_path_nodes)))
            else:
    ...
    assert (len(all_path_nodes)==len(start_nodes)), "len(all_path_nodes) = "+`len(all_path_nodes)`+" len(start_nodes) = "+`len(start_nodes)`
    np.savetxt("dataset_new/start_nodes.txt", np.array(start_nodes), delimiter = " ", fmt = "%s")
    np.savetxt("dataset_new/goal_nodes.txt", np.array(goal_nodes), delimiter = " ", fmt = "%s")
    np.savetxt("dataset_new/occ_grid.txt", np.array(occ_grid), delimiter = " ", fmt = "%s")
